    ...
    private function _do_mobile_login() {
        ...
        if (!empty($result)) {
            if (sp_compare_password($password, $result['user_pass'])) {
                ...
                $this->ajaxReturn(['status' => 1, 'msg' => '登录验证成功！']);
//                $this->success("登录验证成功！", $redirect);
            } else {
//                $this->error("密码错误,请重新登录！",U("user/login/index"));
                $this->ajaxReturn(['status' => 0, 'msg' => '密码错误,请重新输入！']);
            }
        } else {
//            $this->error("用户名不存在,请重新登录！",U("user/login/index"));
            $this->ajaxReturn(['status' => 0, 'msg' => '用户名不存在,请重新输入！']);
        }
    }
